...
const ResourceCard = ({ item, isSaved, onToggleSave }: ResourceCardProps) => {
    ...
    return (
        <div className={`bg-white p-5 rounded-[32px] mb-4 shadow-sm border-2 transition-all duration-300 relative group overflow-hidden ${isSaved ? 'border-indigo-100 shadow-indigo-100/50' : 'border-slate-50 hover:border-slate-100'}`}>
            {isSaved && (
            ...
            <div className="flex justify-between items-start mb-4">
                ...
                <button
                    ...
                    title={isSaved ? "Remove from My Bridge" : "Pin to My Bridge"}
                >
            ...
            <p className="text-sm text-slate-600 mb-5 leading-relaxed font-medium">{item.description}</p>

            {/* Iconic Details (Phase 8) */}
            <div className="grid grid-cols-2 gap-2 mb-6">
                <div className="bg-slate-50 p-3 rounded-2xl flex items-center gap-3 border border-slate-100">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-700">Open Now</p>
                    </div>
                </div>
                <div className="bg-slate-50 p-3 rounded-2xl flex items-center gap-3 border border-slate-100">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-700 truncate">{item.transport || 'Walkable'}</p>
                    </div>
                </div>
            </div>
            ...
            {expanded && (
                <div className="bg-slate-50 p-5 rounded-2xl mb-6 animate-fade-in text-[10px] border border-slate-100 space-y-2">
                    ...
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d, i) => (
                        <div key={d} className={`flex justify-between py-1 ${i === new Date().getDay() ? 'font-black text-indigo-600' : 'text-slate-500 font-bold'}`}>
            ...
            <div className="flex gap-3">
                ...
                >
                    {expanded ? 'Hide Details' : 'Full Hours'}
                </button>
